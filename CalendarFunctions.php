<?php 
	...
	function draw_small_month($month,$year){ //this is a trimed down version of draw_month -Matt
		...
		$calendar.= '<tr><th class="monthtitle">'.implode('</th><th class="monthtitle">',$days).'</th></tr>';
		$days_in_month = date('t',mktime(0,0,0,$month,1,$year));
		$days_in_this_week = 1;
		$day_counter = 0;
		$dates_array = array();
		...
		$daysleft = (35-$running_day);
		if(($month == 2) && ($days_in_month != 29)): //fuck Feb
			$calendar.='</tr><tr>';
		endif;
		for($i = 1; $i < $daysleft+1; $i++): //fills in empty boxes at bottom
	...
	function draw_year($year){ //uses draw_small_month to show whole year -Matt
		...
		for($i = 1; $i < 4; ($i++)):
			$table.='<tr><td>'.draw_small_month($i*$i,$year).'</td><td>'.draw_small_month($i*$i+1,$year).'</td><td>'.draw_small_month($i*$i+2,$year).'</td><td>'.draw_small_month($i*$i+3,$year).'</td></tr>';
		endfor;
